...
G.$3 = ->
    ...
    .then -> H.out 'Tu vista se encoje, es sumamente difícil ver.', 10
    .then -> H.out 'Pero sigues andando.', 10
    .then -> H.newLine()
    .then -> H.out 'Estás perdido.', 3
    .then -> H.out 'Mientras estabas vagando, notas que esto es
    ...
    .then -> H.newLine(10)
    .then -> H.out 'Ya no puedes ver.'
    .then -> H.out 'Estás cansado, pero no puedes dormir.', 3
    .then -> H.out 'Ya no sientes los pies.', 3
    .then -> H.out 'Pero sigues andando.', 3
    .then -> H.newLine(10)
...
G.$3_classroom = ->
    ...
    if chance
        if G.state.wa is true
            ...
            .then -> H.out 'Junto a tí hay un papel.', 3
            .then -> H.newLine(3)
            .then -> H.choice({
                content: "¿Qué decides hacer con el papel?"
                choices: [{
...
G.wa = ->
    ...
    H.thenChain()
    .then -> H.newLine()
    .then -> H.out 'Recojes el papel y empiezas a leerlo.'
    .then -> H.out 'En él, hay tres letras y una mapa garabateada.', 3
    ...
    .then -> H.choice({
        ...
        choices: [{
            ...
            callback: ->
                H.thenChain()
                .then -> H.newLine()
                .then -> H.out 'Ahí dejas el papel, y abandonas la habitación
                    mediante la puerta que entraste.', 3
                .then -> H.out 'Entras al pasillo, y comienzas a
                    deambular.'
                .then -> H.wait 3
                .then -> G.$3()
        }]
...
G.ke = ->
    ...
    .then -> H.out 'También hay otra mapa garabateada atrás del papel.', 3
...
G.ke_room = ->
    ...
    .then -> H.out 'Tu vista se encoje, es difícil ver, pero continúas
...
G.end = ->
    ...
    .then -> H.out 'Ya es demaciado tarde para crear algo, pero aún
...
G.no = ->
    ...
    .then -> H.outRaw '<em>Olvidarás</em> que todo esto pasó.', 3
    ...
    .then -> H.out 'Terminalo.', 3
    .then -> H.out 'Cierra la página.', 3
    .then -> H.out 'Cerrandola acabará con todo esto.', 3
    .then -> H.out 'Tú ni yo tendríamos por qué preocuparnos por esto.', 3
    .then -> H.out 'Asi que', 3
    .then -> H.out 'Terminalo.', 3
    .then -> H.wait 10
    .then -> H.newLine()
    .then -> H.choice({
