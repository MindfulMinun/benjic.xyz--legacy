...
CURRENT_CACHE = '{{ jekyll.environment }}::{{ site.time | slice: 0, 19 }}'
...
retrieveFromCache = (request) ->
    ...
    .then (matching) ->
        return matching or Promise.reject 'no-match'
        ...
        event.respondWith(
            ...
            .then (response) ->
                ...
                if response
                    ...
                    (event.request.mode is "navigate") or
                    event.request.headers.get("accept").includes "text/html"
                )
                    #! If it’s not in the cache and we're requesting an HTML
                    #! file, try that with the offline fallback
                    fetch(event.request).catch -> caches.match offline_url
                else
                    #! If all else fails, just hit the network and
                    #! hope for the best.
                    fetch event.request
        )
